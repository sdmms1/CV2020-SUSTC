...
def get_tiny_images(image_paths):
    ...
    for e in image_paths:
        picture = load_image_gray(e)

        picture = cv2.resize(picture, (16, 16))
        feat = picture.flatten("A")
        feat -= feat.mean()
        feat /= max(abs(feat))
...
def build_vocabulary(image_paths, vocab_size):
    ...
    #############################################################################
    n = len(image_paths)
    step, sample_num = 25, 30

    selected_features = np.zeros((n * sample_num, dim))
    for i in range(n):
        frame, SIFT_features = vlfeat.sift.dsift(load_image_gray(image_paths[i]), fast=True, step=step)
        sample = np.random.choice(a=len(SIFT_features), size=sample_num, replace=False)
        selected_features[i * sample_num:(i + 1) * sample_num] = SIFT_features[sample]

    vocab = vlfeat.kmeans.kmeans(selected_features.astype('float32'), vocab_size)
...
def get_bags_of_sifts(image_paths, vocab_filename):
    ...
    for e in image_paths:
        ...
        frame, SIFT_features = vlfeat.sift.dsift(load_image_gray(e), fast=True, step=step)

        idx = vlfeat.kmeans.kmeans_quantize(SIFT_features.astype("float32"), vocab)
        for i in idx:
            feat[i] += 1
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i in range(len(categories)):
        ...
        svm = svms[category]
        label = np.array([1 if e == category else -1 for e in train_labels])
        svm.fit(train_image_feats, label)
        W.append(svm.coef_)
        B.append(svm.intercept_)
    W, B = np.array(W), np.array(B)
    print(W.shape, B.shape)

    for e in test_image_feats:

        evaluation = []
        for i in range(len(categories)):
            evaluation.append(np.dot(W[i, 0], e) + B[i])

        test_labels.append(categories[evaluation.index(max(evaluation))])
