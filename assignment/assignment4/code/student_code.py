...
def get_positive_features(train_path_pos, feature_params):
    ...
    for i in range(len(positive_files)):
        img = load_image_gray(positive_files[i])
        feats[i] = vlfeat.hog.hog(img, cell_size).flatten()
...
def get_random_negative_features(non_face_scn_path, feature_params, num_samples):
    ...
    gap = win_size // 2
    for e in negative_files:
        img = load_image_gray(e)

        dim = img.shape
        for scale in [1, 0.85, 0.7, 0.55, 0.4]:
            x, y = int(scale * dim[0]), int(scale * dim[1])

            if x >= win_size and y >= win_size:
                resized_img = cv2.resize(img, (y, x), interpolation=cv2.INTER_LINEAR)
                for i in range((x - win_size) // gap):
                    for j in range((y - win_size) // gap):
                        sample_img = resized_img[i * gap:i * gap + win_size, j * gap:j * gap + win_size]
                        feats.append(vlfeat.hog.hog(sample_img, cell_size).flatten())

    feats = np.array(feats)[np.random.choice(len(feats), size=num_samples, replace=False)]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def train_classifier(features_pos, features_neg, C):
    ...
    labels = np.hstack((np.ones(features_pos.shape[0]), -1 * np.ones(features_neg.shape[0])))
        ...
        for scale in [1, 0.85, 0.7, 0.55, 0.4]:
            ...
            if x >= win_size and y >= win_size:
                ...
                for i in range((x - win_size) // gap):
                    for j in range((y - win_size) // gap):
                        sample_img = resized_img[i * gap:i * gap + win_size, j * gap:j * gap + win_size]
                        HOG = vlfeat.hog.hog(sample_img, cell_size).flatten()
...
def run_detector(test_scn_path, svm, feature_params, verbose=False):
    ...
    for idx, im_filename in enumerate(im_filenames):
        ...
        # cur_bboxes = np.hstack([cur_x_min, cur_y_min, (cur_x_min + np.random.rand(15, 1) * 50).astype('int'),
        ...
        for scale in (1 - 0.04 * np.array(range(25))):

            x, y = int(scale * im_shape[0]), int(scale * im_shape[1])
            if x <= win_size or y <= win_size:
                break

            resized_img = cv2.resize(im, (y, x))

            for i in range((x - win_size) // gap):
                for j in range((y - win_size) // gap):
                    sample_img = resized_img[i * gap:i * gap + win_size, j * gap:j * gap + win_size]
                    HOG = vlfeat.hog.hog(sample_img, cell_size).flatten()
                    v = svm.decision_function([HOG])[0]

                    if v >= threshold:
                        coordinate = np.array([j * gap, i * gap, j * gap + win_size, i * gap + win_size], np.float64)
                        origin_coordinate = (coordinate / scale).astype(np.int16)
                        cur_bboxes.append(origin_coordinate)
